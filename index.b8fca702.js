...
class Game {
    ...
    moveRight() {
        ...
        const updatedState = reversedState.map((row)=>this.applyMove(row).reverse());
    ...
    moveDown() {
        ...
        const newState = rotatedLocaleState.map((row)=>this.applyMove([
                ...row
            ]).reverse());
        const rotatedLeftState = this.rotateLeft(newState);
        this.updateGameState(rotatedLeftState);
    ...
    getEmptyCells() {
        ...
        return matrix.flatMap((row, rowIndex)=>row.map((cel, colIndex)=>cel === 0 ? [
                    rowIndex,
                    colIndex
